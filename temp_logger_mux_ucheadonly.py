...
# find mux channel for UC head        
for ch in range(1, 9):
    try:
        if config[sim925_slot][ch]['Thermometer name'] == 'UC Head':
            uchead_ch = ch
    except KeyError:
        pass

# set mux to UC head channel
muxer = sim925_interface(ip, port, sim925_slot)
muxer.set_mux_chan(uchead_ch)
muxer.close()
time.sleep(.5)
    ...
    try:
        ...
        for slot in slots:
            ...
            if module == 'SIM921':
                ...
                name = config[sim925_slot][uchead_ch]['Thermometer name']
                cal_file = config[sim925_slot][uchead_ch]['Calibration file']
